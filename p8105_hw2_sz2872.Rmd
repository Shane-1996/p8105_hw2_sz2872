...
After load the .csv file, the dataset is named `transit_df`. The next step following is unifing the variables' names using `janitor::clean_names`. Then, using the `select` function to pick the variables as order. Moving to the next part, I apply the `mutate` function for the purpose of convert the entry variable from character to a logical variable. The last step is converting variables (route8 tp route11) from double variables to character variables. 
